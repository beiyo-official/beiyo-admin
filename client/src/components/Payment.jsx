...
const Payment = () => {
  ...
  useEffect(() => {
    const fetchPaymentsAndResidents = async () => {
      try {
        // Fetch payments
        const paymentsResponse = await axios.get('https://beiyo-admin.in/api/dashboard/payments');
        ...
        const residentPromises = paymentsData.map(async (payment) => {
          try {
            const residentResponse = await axios.get(`https://beiyo-admin.in/api/dashboard/payment/user/${payment._id}`);
